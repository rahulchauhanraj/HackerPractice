...
public class TimeSeriesQueries {
    ...
    static Map<Integer, TimeSeriesNode> timeSeriesNodes = new LinkedHashMap<>();
    ...
    public static void main(String[] args) {
        ...
        for(int a = 0; a < n; a++){
            ...
            timeSeriesNodes.put(a, node);
        }

        Integer max = -1;
        for(int i = n-1; i >= 0; i--){
            TimeSeriesNode node = timeSeriesNodes.get(i);
        ...
        for(int a0 = 0; a0 < q; a0++){
            ...
            if(type == 1){
                ...
                if(value != -1) {
                    value = timeSeriesNodes.get(value).time;
                }
                System.out.println(value);
            }else {
                int value = getMaxValueIndexForPrice(v);
                if(value != -1) {
                    value = timeSeriesNodes.get(value).maxPriceRemaining;
                }
                System.out.println(value);
            }
        }
    ...
    private static int getMaxValueIndexForPrice(int value){
        return findMaxValueIndex(0, timeSeriesNodes.size()-1, value);
    }

    private static int findMaxValueIndex(int start, int end, int value){
        while(end >= start){
            if(start == end){
                if(timeSeriesNodes.get(start).time < value){
                    return -1;
                }else {
                    return start;
                }
            ...
            if(start+1 == end){
                ...
                if(timeSeriesNodes.get(start).time < value && timeSeriesNodes.get(end).time >= value){
                    return end;
                }
                return -1;
            }
            ...
            if(timeSeriesNodes.get(mid).time < value){
    ...
    private static int getMinTimeIndexForPrice(int value){
        return findMinTimeIndex(0, timeSeriesNodes.size()-1, value);
    }

    private static int findMinTimeIndex(int start, int end, int value){
        while(end >= start){
            if(start == end){
                if(timeSeriesNodes.get(start).maxPriceSoFar < value){
                    return -1;
                }else {
                    return start;
                }
            ...
            if(start+1 == end){
                ...
                if(timeSeriesNodes.get(start).maxPriceSoFar < value && timeSeriesNodes.get(end).maxPriceSoFar >= value){
                    return end;
                }
                return -1;
            }
            ...
            if(timeSeriesNodes.get(mid).maxPriceSoFar < value){
